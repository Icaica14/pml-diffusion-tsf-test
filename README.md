...
🟠 **Sandbox — building the model ladder on the Exchange dataset.** The blueprint lives in [`docs/`](docs/); the first rungs are now code. Already in place: the **data contract** (`src/data/` — contiguous temporal split, train-only scaling, per-split windowing) with an offline test suite; a figure-backed **EDA of Exchange**; the **evaluation module** (point + probabilistic metrics — MAE/RMSE/MASE, CRPS, interval coverage); and the **M0 seasonal-naive baseline**, whose first row is already in [`results/registry.csv`](results/registry.csv). Next rung: **M1 (ARIMA/ETS)**.

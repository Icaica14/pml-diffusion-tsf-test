...
🟠 **Sandbox — building the model ladder on the Exchange dataset.** The blueprint lives in [`docs/`](docs/); the first rungs are now code. Already in place: the **data contract** (`src/data/` — contiguous temporal split, train-only scaling, per-split windowing) with an offline test suite; a figure-backed **EDA of Exchange**; the **evaluation module** (point + probabilistic metrics — MAE/RMSE/MASE, CRPS, interval coverage); the **M0 seasonal-naive baseline**; and now the **M1 ARIMA baseline** (per-channel auto-order, leakage-free per-window conditioning, Gaussian predictive bands); and **M2 DeepAR** (GluonTS, trained on Colab/GPU via [`notebooks/colab_m2_deepar.ipynb`](notebooks/colab_m2_deepar.ipynb)) — all three rows are in [`results/registry.csv`](results/registry.csv). Honest findings so far: M1 *ties* M0 on point accuracy (MASE ≈ 4.52 — an optimal linear forecast collapses to persistence on this unit-root series) but is **better-calibrated** (90% coverage ≈ 0.93 vs 0.86); and **M2 DeepAR *under*performs both** (MASE ≈ 9.2, 90% coverage ≈ 0.76) — the expected outcome on a near-random-walk series, where a flexible learned model has little real structure to exploit and the naive forecast is famously hard to beat. A targeted ablation (stripping DeepAR's spurious nominal-calendar features, the `deepar_notf` row) did *not* close the gap (MASE ≈ 9.8, coverage ≈ 0.72), confirming the shortfall is a structural model–data mismatch, not a covariate artifact. The decisive test of the deep and diffusion models is the **structured primary dataset**, not this sandbox. Next: **M3 (TimeGrad)**, the conditional-diffusion centerpiece.

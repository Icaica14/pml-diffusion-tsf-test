...
🟠 **Sandbox — building the model ladder on the Exchange dataset.** The blueprint lives in [`docs/`](docs/); the first rungs are now code. Already in place: the **data contract** (`src/data/` — contiguous temporal split, train-only scaling, per-split windowing) with an offline test suite; a figure-backed **EDA of Exchange**; the **evaluation module** (point + probabilistic metrics — MAE/RMSE/MASE, CRPS, interval coverage); the **M0 seasonal-naive baseline**; and now the **M1 ARIMA baseline** (per-channel auto-order, leakage-free per-window conditioning, Gaussian predictive bands); **M2 DeepAR** (GluonTS, trained on Colab/GPU via [`notebooks/colab_m2_deepar.ipynb`](notebooks/colab_m2_deepar.ipynb)); and now **M3 TimeGrad**, the conditional-diffusion centerpiece (PyTorchTS, trained on Colab/GPU via [`notebooks/colab_m3_timegrad.ipynb`](notebooks/colab_m3_timegrad.ipynb)) — **all four rows** are in [`results/registry.csv`](results/registry.csv), so the ladder is now complete on the sandbox. Honest findings: M1 *ties* M0 on point accuracy (MASE ≈ 4.52 — an optimal linear forecast collapses to persistence on this unit-root series) but is **better-calibrated** (90% coverage ≈ 0.93 vs 0.86); and **M2 DeepAR *under*performs both** (MASE ≈ 9.2, 90% coverage ≈ 0.76) — the expected outcome on a near-random-walk series, where a flexible learned model has little real structure to exploit and the naive forecast is famously hard to beat. A targeted ablation (stripping DeepAR's spurious nominal-calendar features, the `deepar_notf` row) did *not* close the gap (MASE ≈ 9.8, coverage ≈ 0.72), confirming the shortfall is a structural model–data mismatch, not a covariate artifact. And the diffusion centerpiece **M3 TimeGrad lands the same way**: MASE ≈ 9.8 (tied with DeepAR, far above the naive), and its CRPS ≈ 0.015 is in fact the *worst* of the five — it recovers near-nominal coverage (50%/90% ≈ 0.42/0.84, vs DeepAR's 0.29/0.76) only by drawing **much wider, more diffuse** bands (90% width ≈ 0.078 vs ≈ 0.045 for everyone else), not sharper ones — at ≈ **25× the predict cost** of the deep baseline (740 s vs ~30 s: the 100-step × 100-sample diffusion sampling the project's thesis is about). The verdict on the sandbox is now complete and consistent: on a near-random-walk series **no model — classical, deep, or diffusion — beats persistence**, and flexibility buys cost, not accuracy. The decisive test of the deep and diffusion models is the **structured primary dataset**, not this sandbox.
...
**Stato:** 🟠 sandbox personale, sto costruendo la scala dei modelli sul dataset **Exchange**. Già pronti: il **contratto dei dati** (`src/data/` — split temporale contiguo, scaling solo su train, windowing per-split) con la sua suite di test offline; una **EDA di Exchange** corredata di figure; il **modulo di valutazione** (metriche puntuali + probabilistiche — MAE/RMSE/MASE, CRPS, copertura degli intervalli); la **baseline M0 seasonal-naive**; la **baseline M1 ARIMA** (ordine auto per canale, condizionamento per-finestra senza leakage, bande predittive gaussiane); **M2 DeepAR** (GluonTS, addestrato su Colab/GPU tramite [`notebooks/colab_m2_deepar.ipynb`](notebooks/colab_m2_deepar.ipynb)); e ora **M3 TimeGrad**, il cuore a diffusione condizionata (PyTorchTS, addestrato su Colab/GPU tramite [`notebooks/colab_m3_timegrad.ipynb`](notebooks/colab_m3_timegrad.ipynb)) — **tutte e quattro le righe** sono in [`results/registry.csv`](results/registry.csv), quindi la scala dei modelli sulla sandbox è ora completa. Risultati onesti: sulla serie Exchange quasi-random-walk M1 *pareggia* M0 sull'accuratezza puntuale (MASE ≈ 4.52 — su una serie con radice unitaria il forecast lineare ottimo collassa sulla persistenza) ma offre un'incertezza **meglio calibrata** (copertura al 90% ≈ 0.93 contro 0.86); e **M2 DeepAR fa *peggio* di entrambe** (MASE ≈ 9.2, copertura al 90% ≈ 0.76) — l'esito atteso su una serie quasi-random-walk, dove un modello flessibile appreso ha poca struttura reale da sfruttare e la baseline ingenua è notoriamente difficile da battere. Un'ablazione mirata (rimozione delle feature di calendario nominali spurie, la riga `deepar_notf`) *non* ha chiuso il divario (MASE ≈ 9.8, copertura ≈ 0.72), confermando che il problema è un disallineamento strutturale modello–dati, non un artefatto delle covariate. E il cuore a diffusione **M3 TimeGrad finisce allo stesso modo**: MASE ≈ 9.8 (pari a DeepAR, molto sopra la baseline ingenua), e il suo CRPS ≈ 0.015 è anzi il *peggiore* dei cinque — recupera una copertura quasi nominale (50%/90% ≈ 0.42/0.84, contro 0.29/0.76 di DeepAR) solo disegnando bande **molto più larghe e diffuse** (ampiezza al 90% ≈ 0.078 contro ≈ 0.045 di tutti gli altri), non più nitide — al costo di un predict ≈ **25× quello della baseline deep** (740 s contro ~30 s: il campionamento a diffusione 100 passi × 100 traiettorie su cui verte la tesi del progetto). Il verdetto sulla sandbox è ora completo e coerente: su una serie quasi-random-walk **nessun modello — classico, deep o a diffusione — batte la persistenza**, e la flessibilità compra costo, non accuratezza. Il test decisivo per i modelli deep e a diffusione è il **dataset primario strutturato**, non questa sandbox. Il piano completo è in [`docs/IMPLEMENTATION_PLAN_IT.md`](docs/IMPLEMENTATION_PLAN_IT.md) — include un'appendice che spiega a fondo la fase sperimentale (prima la metodologia, poi le istruzioni operative passo-passo).
